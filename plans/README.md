...
|------|-------|----------|--------|------------|--------|
| 082 | Recover from missing Full Disk Access without a restart; `make verify-db` gates install; actionable FDA fix text; incident timeline recorded | P1 | S-M | none | DONE 2026-09-02, branch `advisor/082-database-access-self-heal`; 436/0. `make verify-db` ✓ against live 1.6.0 pid 47025 (`accessible`, `~/Library/Messages/chat.db`). Added `MCP-Protocol-Version: 2025-11-25` on the diagnose call (`HTTPTransportIntegrationTests` requires it after initialize). Step 6 FDA toggle not run: would risk the live signed 1.6.0 service; incident already shows grants apply only on launch. |

...
|------|-------|----------|--------|------------|--------|
| 082 | Recover from missing Full Disk Access without a restart; `make verify-db` gates install; actionable FDA fix text; incident timeline recorded | P1 | S-M | none | DONE 2026-09-02, merged to main (`7563568`, branch `advisor/082-database-access-self-heal`; 436/0). `make verify-db` ✓ against live 1.6.0 pid 47025. Added `MCP-Protocol-Version: 2025-11-25` on the diagnose call. Step 6 FDA toggle not run: would risk the live signed 1.6.0 service; incident already shows grants apply only on launch. |
| 085 | Stage `send` attachments through a symlink-safe file handle into a 0700 directory (lstat every component, trusted `/tmp` `/var` `/etc` aliases) | P1 | M | none | DONE 2026-09-02, merged to main (`cb74bb8`, branch `advisor/085-symlink-safe-attachment-staging`; 447/0). Symlink/TOCTOU tests use `temporaryDirectory` scratch only. Step 6 Messages probe skipped (optional). |
| 079 | Typed delivery disposition for `send`: `disposition` (`completed` / `not_started` / `may_have_completed`) and `retry_safe`; `confirmed` defined as chat.db row match within the verification window | P2 | M | none | DONE 2026-09-02, merged to main (`f4602c3`, branch `advisor/079-send-delivery-disposition`; 464/0). Live `make install` skipped. classifySendStderr now matches `not authorized` and keeps original case on the fallback line. |
| 083 | URL preview balloon rows: pin single-row behaviour, fix `has:"links"` blind spot, hide `pluginPayloadAttachment` rows (imsg-style 90 s coalescing deferred: 0 of 1,121 balloon rows in 365 d pair with a text row on this DB) | P2 | S-M | none | DONE 2026-09-02, merged to main (`085dd5a`, branch `advisor/083-url-preview-coalescing`; 474/0). Premise re-check: balloon_rows_365d=1115, paired_with_url_text_90s=0; imsg coalescing not ported. |
| 081 | Probe the chat.db schema at open; `SchemaCapabilities` flags guard `thread_originator_guid`, `date_edited`, `associated_message_emoji`; `database.features` in diagnose | P2 | M | 082 (shared Diagnose edits) | DONE 2026-09-02, merged to main (`24e2b83`, branch `advisor/081-schema-capability-flags`; 486/0). Guarded only the three columns. Stdio live diagnose features not captured (initialize-only on stdin EOF); unsigned debug binary. |
| 084 | Never request Contacts authorization from a headless process; refresh the cache on change and TTL; clear it on revoke; `not_requested_headless` in diagnose | P2 | M | 082 (shared Diagnose edits) | DONE 2026-09-02, merged to main (`206f074`, branch `advisor/084-headless-contacts-access-policy`; 500/0). Policy tests do not touch CNContactStore. `--request-contacts-access` not run. Launchd 1.6.0 diagnose: contacts authorized, loaded 1666. Debug HTTP :8099 same. |
| 080 | Live inbox: `get_messages_since` ROWID cursor tool (layer 1), then a WAL-aware chat.db watcher pushing `notifications/imessage/new_messages` over SSE (layer 2, optional) | P2 | M / L | 083 (balloon rows affect cursor consumption) | DONE 2026-09-02. Layer 1 merged (`5c4f723`, `advisor/080-live-inbox`; 513/0). Live smoke :8097: omit cursor next=current=234009 (= sqlite3 MAX); since 233959 → 37 msgs, ascending, filtered_hidden=2, next=current=234009. Layer 2 merged (`1cc1890`, `advisor/080b-live-inbox-watcher`; 520/0, 1 skipped). `Server.notify` exists at swift-sdk 0.12.1 `Sources/MCP/Server/Server.swift:379` and already broadcasts id-less payloads via `HTTPTransport.handleServerResponse`. WAL-rotation test XCTSkip: SQLite `disk I/O error` after `-wal` rename. kqueue path fired in `testFiresWhenARowIsInserted` (117 ms, fallback 400 ms). |

Execution 2026-09-02 (local main only, not pushed): 082 `7563568` 436/0; 085 `cb74bb8` 447/0; 079 `f4602c3` 464/0; 083 `085dd5a` 474/0; 081 `24e2b83` 486/0; 084 `206f074` 500/0; 080 layer 1 `5c4f723` 513/0; 080 layer 2 `1cc1890` 520/0 (1 skip). No BLOCKED plan. Live launchd service left on 1.6.0; `make install` not run.

### Findings considered and rejected (2026-09-02)

- imsg URL balloon coalescing/dedupe — 0 of 1,121 balloon rows in 365 d pair with a URL text row (live re-check 1,115 / 0); see 083 Appendix A.

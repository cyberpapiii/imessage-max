...
| 016 | Narrow confirmation gate to risky sends (drop blanket chat-target rule) | P1 | S | 015 | REJECTED (superseded by 017, 2026-06-11 — live debugging showed the gate rule was only half the problem; the elicitation wait itself is the broken path, so both are removed together in one policy change) |
| 017 | Agent-native send contract (remove the confirmation gate and elicitation) | P1 | M | 015 | DONE (2026-06-11, executor commit `74855c4`, merged `834d10c`, 146/146 tests, deployed; live acceptance PASS — no-confirm chat-id send through plug returned `confirmed` with guid instantly, `confirm: true` identical (inert), stable PID, no crash log entries; group-send live check deferred to avoid messaging real recipients — covered by `testGroupChatSendWithoutConfirmSendsImmediately`) |
| 014 | Bound the send-confirmation elicitation wait (plug/Cursor 300s hang) | P1 | S | — | DONE (2026-06-11, commit `5699d84`, reviewed & approved, merged; original 60s timeout lowered to 25s by 015; NOTE: 014's Task.sleep-based timer crashed the launchd service — fixed by 015; the entire elicitation wait is removed by 017) |

...
| 080 | Live inbox: `get_messages_since` ROWID cursor tool (layer 1), then a WAL-aware chat.db watcher pushing `notifications/imessage/new_messages` over SSE (layer 2, optional) | P2 | M / L | 083 (balloon rows affect cursor consumption) | DONE 2026-09-02. Layer 1 merged (`5c4f723`, `advisor/080-live-inbox`; 513/0). Live smoke :8097: omit cursor next=current=234009 (= sqlite3 MAX); since 233959 → 37 msgs, ascending, filtered_hidden=2, next=current=234009. Layer 2 merged (`advisor/080b-live-inbox-watcher`). `Server.notify` exists at swift-sdk 0.12.1 `Sources/MCP/Server/Server.swift:379` and already broadcasts id-less payloads via `HTTPTransport.handleServerResponse`. WAL-rotation test XCTSkip: SQLite `disk I/O error` after `-wal` rename. kqueue path fired in `testFiresWhenARowIsInserted` (117 ms, fallback 400 ms). |

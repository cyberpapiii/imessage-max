...
**2026-09-01, against commit `61e75d9`** (plans 040–055 landed on `main`
2026-09-01, plus follow-up 056). The two previous rounds (2026-08-07,
plans 019–039; 2026-06-11, plans 001–018) are fully merged; their tables
...
**278/278 tests pass at `61e75d9`** (also what CI runs, once plan 040 makes CI
green again). Deploying to the local launchd service (`make install`) is an
...
## Current round (2026-09-01, against `61e75d9`): execution order & status
...
| 060 | CI hang: `swift test --parallel` on macos-26 stuck after HTTP tests | P0 | S | 059 | DONE 2026-09-01. `a98c065` dropped the 24MP noise JPEG; `35b3d63` stopped `process()` on a SOF-spoofed 50MP header; `advisor/060-ci-hang-3` injects a 20ms SessionManager cleanup interval so cancelled 300s `asyncAfter` sleeps do not pin parallel workers (5m03s gaps in run 33571807330). `timeout-minutes: 15` kept. 370/0. |

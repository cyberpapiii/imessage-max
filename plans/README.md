...
| 084 | Never request Contacts authorization from a headless process; refresh the cache on change and TTL; clear it on revoke; `not_requested_headless` in diagnose | P2 | M | 082 (shared Diagnose edits) | DONE 2026-09-02, branch `advisor/084-headless-contacts-access-policy`. Policy tests do not touch CNContactStore. `--request-contacts-access` not run (would prompt); launchd status read from existing diagnose. |

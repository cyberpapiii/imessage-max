...
| 079 | Typed delivery disposition for `send`: `disposition` (`completed` / `not_started` / `may_have_completed`) and `retry_safe`; `confirmed` defined as chat.db row match within the verification window | P2 | M | none | DONE 2026-09-02, branch `advisor/079-send-delivery-disposition`; 464/0. Step 3 live `make install` skipped (operator deploys). classifySendStderr now matches `not authorized` and keeps original case on the fallback line. |

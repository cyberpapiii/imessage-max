...
| 080 | Live inbox: `get_messages_since` ROWID cursor tool (layer 1), then a WAL-aware chat.db watcher pushing `notifications/imessage/new_messages` over SSE (layer 2, optional) | P2 | M / L | 083 (balloon rows affect cursor consumption) | DONE layer 1 2026-09-02, merged to main (`5c4f723`, branch `advisor/080-live-inbox`; 513/0). Live smoke :8097 debug HTTP: omit cursor next_rowid=current_rowid=234009 (= sqlite3 MAX); since 233959 → 37 msgs, ascending, filtered_hidden=2, next_rowid=current_rowid=234009. SQL LIMIT looped in batches so consumed filtered/orphan rows cannot starve a page. Layer 2 on `advisor/080b-live-inbox-watcher`. |

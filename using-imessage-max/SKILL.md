...
- Sends execute immediately when the destination is exact; there is no confirmation step and the deprecated `confirm` parameter is ignored. Check the returned `status`: `confirmed` is verified delivery, `uncertain` means follow up with `get_messages`, `mismatch` means it landed in the wrong chat — tell the user. `pending_confirmation` appears only for file attachments whose transfer hasn't finished; it is not a request to retry.
